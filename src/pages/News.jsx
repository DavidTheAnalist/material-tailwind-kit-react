  ...
  const slides = [
    {
      ...
      imageUrl: "/img/Bus Trend.webp",
    },
    {
      title: "Exploring the Future of AI",
      description: "A deep dive into the advancements in AI and how it's shaping industries worldwide.",
      imageUrl: "/img/AI 2.png",
    },
  ];

  const categories = [
    { name: "Business", description: "AI helps businesses streamline operations, enhance customer experiences, and make data-driven decisions. With AI-powered tools, businesses can automate tasks, improve productivity, and predict market trends more accurately.", imageUrl: "/img/Business.webp" },
  ...
  return (
    ...
    <section className="container mx-auto px-4 py-8">
      ...
      <div className="relative mb-8 animate-fadeIn">
        <Typography variant="h1" className="text-5xl font-bold text-gray-900">
      ...
      {/* Intro Section */}
      <div className="bg-white rounded-xl shadow-lg p-6 mb-8 animate-fadeInUp shadow-2xl border-[#3B82F6] bg-white/95 p-8 rounded-xl transform transition-all duration-700 hover:scale-110 hover:shadow-[0_0_50px_rgba(255,182,193,0.7)]">
      ...
      {/* Carousel */}
<div className="relative w-full h-[500px] rounded-xl overflow-hidden mb-8 shadow-lg hover:scale-110 transform transition-all duration-700">
  {/* Carousel Content */}
  ...
  <button
    className="absolute top-1/2 left-4 transform -translate-y-1/2 rounded-full bg-white p-4 shadow-lg opacity-50 hover:opacity-75 transition-opacity z-20"
  ...
  <button
    className="absolute top-1/2 right-4 transform -translate-y-1/2 rounded-full bg-white p-4 shadow-lg opacity-50 hover:opacity-75 transition-opacity z-20"
...
<section className="container mx-auto px-4 py-8">
      ...
      <div className="bg-white rounded-xl shadow-lg p-6 mb-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-1 md:grid-cols-3 gap-8">
          {categories.map((category, index) => (
            <div key={index} className="hover:scale-110 transform transition-all duration-700 flex flex-col items-center bg-white-100 p-6 rounded-xl max-w-4xl mx-auto glassmorphism-extreme bg-[#4239c2]/10 backdrop-lg p-6 rounded-xl mt-12 relative flex items-center justify-center py-19 bg-black rounded-xl border border-[#f370e8]/30 transition-all duration-700 hover:shadow-[0_0_50px_rgba(255,182,193,0.9)]">
              <div className="w-full h-full rounded-lg overflow-hidden mb-4 transform transition-transform duration-300 hover:scale-105">
                <img
              ...
              {selectedCategory === category.name ? (
                <Typography variant="paragraph" className="text-lg text-gray-600 mb-4 text-center">
                  {category.description}
                </Typography>
              ) : (
                <Button
                  color="red"
                  className="px-6 py-3 text-lg transition-transform duration-800 hover:scale-105"
      ...
      <div className="relative w-full h-[300px] rounded-xl overflow-hidden mb-8 shadow-lg animate-fadeInUp hover:scale-110 transform transition-all duration-700 hover:shadow-[0_0_50px_rgba(255,182,193,0.7)]">
        <img
          src="/img/Insights.jfif"
          alt="Expert Insights"
          className="w-full h-full object-cover"
        />
        <div className="absolute bottom-0 left-0 w-full bg-gradient-to-t from-black to-transparent p-6">
      ...
      {/* Newsletter Section */}
      <div className="relative w-full h-[450px] bg-gray-900 rounded-xl overflow-hidden shadow-lg">
        <img
          src="/img/pattern.png"
        ...
        <div className="absolute bottom-6 right-6 p-6 animate-fadeInUp">
          <Typography variant="h2" className="text-xl font-semibold text-white mb-2">
        ...
        <div className="absolute top-6 left-6 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4 sm:gap-6 text-white text-sm sm:text-base">
          {footerLinks.map((link, index) => (
            <a
              key={index}
              href={link.href}
              className="hover:text-red-400 mx-auto transition-colors"
            >
              {link.text}
            </a>
          ))}
        </div>
        <div className="absolute sm:top-4 sm:right-6 sm:flex sm:gap-4 sm:bottom-auto sm:left-auto bottom-6 right-6 flex gap-3">
          {socialIcons.map((icon, index) => (
            ...
            >
              <img src={icon.src} alt={icon.alt} className="w-6 h-5" />
            </a>
          ))}
        </div>
      </div>
